---
title: "개발자 분노의 대상, Github Copilot의 새 요금제는?"
date: "2026-05-31T13:19:04.695139"
category: "tech"
tags:
  - tech
  - What
  - joke
  - Github
quality_score: 88.0
cover_image: "https://images.unsplash.com/photo-1762330470070-249e7c23c8c0?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w5NjUzMTl8MHwxfHNlYXJjaHwxfHxXaGF0JTI1MjBqb2tlJTI1MjBHaXRodWIlMjUyMHRlY2h8ZW58MHwwfHx8MTc4MDIzMzU1NXww&ixlib=rb-4.1.0&q=80&w=1080"
cover_image_alt: "개발자 분노의 대상, Github Copilot의 새 요금제는? — Photo by Zulfugar Karimov on Unsplash (https://unsplash.com/photos/digital-interface-with-ask-anything-prompt--lZmnpignB8)"
---
![개발자 분노의 대상, Github Copilot의 새 요금제는? — Photo by Zulfugar Karimov on Unsplash (https://unsplash.com/photos/digital-interface-with-ask-anything-prompt--lZmnpignB8)](https://images.unsplash.com/photo-1762330470070-249e7c23c8c0?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w5NjUzMTl8MHwxfHNlYXJjaHwxfHxXaGF0JTI1MjBqb2tlJTI1MjBHaXRodWIlMjUyMHRlY2h8ZW58MHwwfHx8MTc4MDIzMzU1NXww&ixlib=rb-4.1.0&q=80&w=1080)

### Github Copilot란?
Github Copilot는 Microsoft가 소유하는 Github에서 제공하는 인공지능 기반 코드 작성 도구입니다. 이 도구는 개발자들이 코드를 더 빠르고 효율적으로 작성할 수 있도록 도와줍니다.

### 새로운 요금제
Github Copilot는 최근 새로운 토큰 기반 요금제를 도입했습니다. 이 새로운 요금제는 개발자들이 사용하는 코드의 양에 따라 비용을 부과합니다.

#### 요금제 구조
| 토큰 사용량 | 비용 |
| --- | --- |
| 0-1000 토큰 | 무료 |
| 1001-10000 토큰 | $10/월 |
| 10001-50000 토큰 | $50/월 |
| 50001 토큰 이상 | $100/월 |

### 개발자들의 반응
개발자들은 이 새로운 요금제가 너무 비싸고, 개발의 자유를 제한한다고 주장합니다. "What joke Github"는 개발자들의 불만을 대표하는 키워드입니다. 개발자들은 Github Copilot의 새로운 요금제가 개발의 자유를 제한하고, 개발 비용을 증가시킨다고 주장합니다.

### Microsoft의 입장
Microsoft는 Github Copilot의 새로운 요금제가 개발자들에게 더 많은 비용을 부과할 수 있음을 인식하고 있습니다. 그러나 Microsoft는 Github Copilot의 새로운 요금제가 개발자들에게 더 많은 가치를 제공할 수 있음을 주장합니다.

### 결론
Github Copilot의 새로운 요금제는 개발자들의 불만을 일으키고 있습니다. 개발자들은 이 새로운 요금제가 개발의 자유를 제한하고, 개발 비용을 증가시킨다고 주장합니다. 더 많은 정보를 알고 싶다면, Microsoft의 공식 홈페이지를 방문하세요.
