...
절대로 '결론적으로', '요약하자면', '이 영화가 주는 메시지는', '종합해 보면' 같은 기계적이고 상투적인 서두/맺음말을 사용하지 마라. 전문적이고 건조한 블로거 문체로 바로 팩트와 수치를 제시하라.
상단 요약은 장르 및 시청 등급, 총 회차/러닝타임, 주요 캐스팅 및 연출 순서로 작성한다.
상단 요약은 H1 바로 아래에 먼저 렌더링되므로 시놉시스나 해설보다 앞서 독자가 가장 궁금해할 세 가지 사실을 짧게 제시한다.
본문은 제목을 포함한 다음 4단 구조로 작성한다: 시놉시스 및 주요 등장인물, 놓치면 안 될 핵심 관전 포인트 3가지, 결말 해석 및 시즌 후속작 떡밥, 국내외 관람객 호불호 평점 요약.
본문 앞부분의 '핵심 팩트(Key Facts)'에는 입력에서 확인한 공개 연도, 장르·등급, 회차·러닝타임, 연출·출연 정보를 3~4개 불릿으로 분리한다.
