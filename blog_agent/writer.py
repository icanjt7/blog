...
{topic_with_subject} 검색 전에 기준을 세워두면 시간을 꽤 줄일 수 있습니다. 같은 키워드라도 지역, 날짜, 대상 조건에 따라 필요한 정보가 달라지기 때문입니다.

{frame["opening"]}

## 왜 지금 볼 만한가

- 주제 성격: {topic.rationale or frame["rationale"]}
- 먼저 볼 것: {frame["first_check"]}
- 독자 상황: {frame["reader"]}

{topic.keyword}를 볼 때 가장 흔한 실수는 제목만 보고 바로 결론을 내리는 것입니다. 실제로는 대상, 시점, 예외 조건을 나눠 봐야 하고, 후기성 글과 원문 자료의 역할도 다르게 봐야 합니다.
...
| 비용·조건 | {frame["cost_check"]} |
| 예외 | {frame["exception_check"]} |

## 순서대로 확인하기

1. 먼저 {topic.keyword}의 기준일을 확인합니다. 검색 결과가 최신처럼 보여도 본문 안의 날짜가 오래된 경우가 있습니다.
...
{topic.keyword} 관련 정보는 같은 표현이 반복돼도 실제 의미가 다를 수 있습니다. 예를 들어 "신청 가능"은 접수 기간이 열렸다는 뜻일 수도 있고, 특정 대상에게만 가능하다는 뜻일 수도 있습니다. 여행이나 생활 정보에서는 운영 시간과 휴무일, 정책 정보에서는 기준일과 담당 기관, 스포츠 정보에서는 대회 주관처와 엔트리 확정 여부를 분리해서 보는 편이 안전합니다.

## 체크리스트

1. {topic.keyword}의 기준일을 확인했는가
...
{topic.keyword}는 한 번에 결론을 내기보다 기준을 잡고 확인하면 훨씬 덜 헷갈립니다. 지금 할 일은 검색 결과를 많이 여는 것이 아니라, 날짜와 대상 조건을 먼저 좁힌 뒤 원문 또는 운영 주체의 안내를 확인하는 것입니다.
...
"""
    ...
    def _fallback_frame(self, topic: Topic) -> dict[str, str]:
        ...
        if topic.category == "생활":
            return {
                "title": f"{keyword}, 헷갈리는 조건 3가지",
                "excerpt": f"{keyword}를 확인할 때 대상, 준비물, 기준일을 나눠 보는 방법을 정리했습니다.",
                "rationale": "생활 속에서 바로 행동으로 이어지는 검색 주제",
                "opening": "생활 정보는 작은 조건 하나 때문에 결과가 달라집니다. 특히 가족, 신분증, 신청자 본인 여부처럼 서류와 연결되는 주제는 더 그렇습니다.",
                "first_check": "대상자, 준비물, 접수 또는 이용 기준일",
                "reader": "바로 신청하거나 방문하기 전에 빠르게 기준을 잡고 싶은 사람",
                "target_check": "본인, 가족, 미성년자, 대리 신청 가능 여부",
                "timing_check": "접수 기간, 방문 가능 시간, 처리 소요 기간",
                "cost_check": "수수료, 예약비, 추가 준비물, 환불 조건",
                "exception_check": "대리 신청 제한, 지역 제한, 사전 예약 필요 여부",
                "detail_1": "생활 정보는 '가능하다'는 말보다 '누가 어떤 조건에서 가능한가'가 중요합니다. 가족이 대신할 수 있는지, 온라인으로 끝나는지, 현장 방문이 필요한지부터 나눠 보세요.",
                "detail_2": "준비물은 가장 늦게 확인하면 실수가 납니다. 신분증, 가족관계 확인 서류, 사진, 인증 수단처럼 빠지면 다시 방문해야 하는 항목을 먼저 보는 편이 좋습니다.",
            }
        if topic.category == "정책":
            return {
                "title": f"{keyword}, 대상부터 확인하세요",
                "excerpt": f"{keyword} 이슈를 대상, 기준일, 후속 확인 경로 중심으로 정리했습니다.",
        ...
        if topic.category == "스포츠":
            return {
                "title": f"{keyword}, 경기 전 볼 변수 3가지",
                "excerpt": f"{keyword}를 일정, 명단, 관전 포인트 중심으로 빠르게 확인할 수 있게 정리했습니다.",
                ...
                "target_check": "대표팀, 선수, 대회, 리그 중 무엇을 보는지",
                "timing_check": "경기일, 엔트리 발표일, 중계 시간",
                "cost_check": "중계 플랫폼, 유료 시청 여부, 현장 관람 비용",
                "exception_check": "부상, 징계, 일정 변경, 기상 변수",
                "detail_1": "경기 전 정보는 발표 시점이 중요합니다. 예비 명단과 최종 명단, 평가전과 공식전, 현지 시간과 한국 시간을 구분해야 착오가 줄어듭니다.",
                "detail_2": "전술 전망은 해석이고, 일정과 기록은 사실입니다. 두 정보를 분리해서 보면 과장된 예상 글에 휘둘릴 가능성이 줄어듭니다.",
            }
        return {
            "title": f"{keyword}, 가기 전 볼 기준 3가지",
            "excerpt": f"{keyword}를 동선, 비용, 운영 조건 중심으로 확인하는 방법을 정리했습니다.",
