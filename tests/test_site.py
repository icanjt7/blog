...
""",
        ...
        self.assertLess(body.index('id="executive-summary"'), body.index("도입 문장입니다."))
...
"""
    ...
    def test_openapi_product_card_shows_price_without_unapproved_image(self) -> None:
        ...
        self.assertIn('class="toss-cta-button product-recommendation-link"', card)
...
""",
            ...
            not_found_html = (root / "public" / "404.html").read_text(encoding="utf-8")
            ...
            self.assertIn('id="recommended-refresh-button"', index_html)
            self.assertIn("새로운 추천 보기", index_html)
            self.assertIn("refreshButton.addEventListener('click'", index_html)
            self.assertIn("var fresh=candidates.filter", index_html)
            self.assertIn('id="recommended-refresh-status"', index_html)
            ...
            self.assertIn(".recommended-refresh-button {", style)
            self.assertIn(".recommended-grid.is-refreshing {", style)
            self.assertIn(".error-container {", style)
            self.assertIn(".random-posts-grid {", style)
            self.assertIn("앗, 페이지를 찾을 수 없습니다", not_found_html)
            self.assertIn('window.AVAILABLE_POSTS=[', not_found_html)
            self.assertIn("for(var i=copy.length-1;i>0;i--)", not_found_html)
            self.assertIn('id="random-posts-container"', not_found_html)
            self.assertIn('<link rel="stylesheet" href="/style.css">', not_found_html)
            self.assertIn('<a href="/" class="home-btn">', not_found_html)
            self.assertIn('<meta name="robots" content="noindex,follow">', not_found_html)
            self.assertNotIn('src="/assets/img/thumb', not_found_html)
            for label in ("생활", "기술", "정책", "환경", "정치", "스포츠", "핫이슈", "영화"):
                self.assertIn(f">{label}</a>", not_found_html)
            ...
            self.assertNotIn("404.html", static_sitemap)
...
""",
            ...
            self.assertIn('class="toss-cta-button product-recommendation-link"', html)
            css = (root / "public" / "style.css").read_text(encoding="utf-8")
            self.assertIn(".toss-cta-button:active", css)
            self.assertIn("transform: scale(.98)", css)
            self.assertIn("background: #0050ff", css)
